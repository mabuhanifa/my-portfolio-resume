const projects = [
  {
    id: 1,
    name: "Biospc",
    info: "A website for the Biospc, a non-profit organization that provides education and support to the community through the use of technology.",
    info_bullets: [
      "BiosPC is a Full Stack (MERN) web Application.",
      "MongoDB has been used as a Database alongside Node and Express JS.",
      "User Authentication has been implemented using Firebase.",
      "React, React Router, React Hook Form, Tailwind CSS, DaisyUI have been used.",
      "This project is built with react-firebase hooks."
    ],
    tech: [
      { name: "React", bg: "#5ed3f333", text: "#5ED3F3" },
      { name: "Node Js", bg: "#6fae543a", text: "#6FAE54" },
      { name: "Express Js", bg: "#2ea1ff27", text: "#2EA1FF" },
      { name: "MongoDB", bg: "#0fa14c27", text: "##0FA14C" },
      { name: "Firebase", bg: "#f2bf2833", text: "#F2C028" },
      { name: "Tailwind CSS", bg: "#15b4c14e", text: "#15B4C1" },
      { name: "DaisyUI", bg: "#303ebe5e", text: "#4862f5" },
    ],
    img: "https://i.ibb.co/zGtMzNz/biospc.png",
    images: [
      "https://i.ibb.co/ZJQHN6n/Opera-Snapshot-2022-06-11-144541-mern-last-assignment-web-app.png",
      "https://i.ibb.co/DbVc2yb/Opera-Snapshot-2022-06-11-144618-mern-last-assignment-web-app.png",
      "https://i.ibb.co/dMyKhdJ/Opera-Snapshot-2022-06-11-144551-mern-last-assignment-web-app.png",
    ],
    link: {
      live: "https://mern-last-assignment.web.app",
      client: "https://github.com/mabuhanifa/BiosPC-client-side",
      server: "https://github.com/mabuhanifa/BiosPC-server-side",
    },
  },
  {
    id: 2,
    name: "Gadgetarena",
    info: "A website for the Biospc, a non-profit organization that provides education and support to the community through the use of technology.",
    info_bullets: [
      "Gadget Arena is a Full Stack (MERN) web Application.",
      "MongoDB has been used as a Database alongside Node and Express JS.",
      "Firebase has been implemented for User Authentication .",
      "React, React Router, React-Bootstrap, React Toast have been used.",
      "This project is built with react-firebase hooks.",
    ],
    tech: [
      { name: "React", bg: "#5ed3f333", text: "#5ED3F3" },
      { name: "Node Js", bg: "#6fae543a", text: "#6FAE54" },
      { name: "Express Js", bg: "#2ea1ff27", text: "#2EA1FF" },
      { name: "MongoDB", bg: "#0fa14c27", text: "##0FA14C" },
      { name: "Firebase", bg: "#f2bf2833", text: "#F2C028" },
      { name: "React Router", bg: "#c03f4131", text: "#C03F41" },
      { name: "React-Bootstrap", bg: "#303ebe5e", text: "#4862f5" },
    ],
    img: "https://i.ibb.co/g6nMSvb/gadgetarena.png",
    images: [
      "https://i.ibb.co/JtH2HHd/Opera-Snapshot-2022-06-11-144717-mern-assignment-01-web-app.png",
      "https://i.ibb.co/KmSwCVt/Opera-Snapshot-2022-06-11-144828-mern-assignment-01-web-app.png",
      "https://i.ibb.co/QrCKCYS/Opera-Snapshot-2022-06-11-144803-mern-assignment-01-web-app.png",
    ],
    link: {
      live: "https://mern-assignment-01.web.app",
      client: "https://github.com/mabuhanifa/gadget-arena-client",
      server: "https://github.com/mabuhanifa/gadget-arena-server",
    },
  },
  {
    id: 3,
    name: "Snapmaster",
    info: "A website for the Biospc, a non-profit organization that provides education and support to the community through the use of technology.",
    info_bullets: [
      "SnapMaster is a React Single Page Application.",
      "This website is built with React and React Router.",
      "This project contains private route.",
      "React, React Router, React-Bootstrap, React Toast have been used.",
      "This project is built with react-firebase hooks.",
    ],
    tech: [
      { name: "React", bg: "#5ed3f333", text: "#5ED3F3" },
      { name: "React Router", bg: "#c03f4131", text: "#C03F41" },
      { name: "React Bootstrap", bg: "#2ea1ff27", text: "#2EA1FF" },
      { name: "Firebase", bg: "#f2bf2833", text: "#F2C028" },
      { name: "React Toastify", bg: "#303ebe5e", text: "#4862f5" },
    ],
    img: "https://i.ibb.co/q7CCVm4/snapmaster.png",
    images: [
      "https://i.ibb.co/8bP2pk7/Opera-Snapshot-2022-06-11-144907-react-firebase-assignment-09-firebaseapp-com.png",
      "https://i.ibb.co/hCnF8F4/Opera-Snapshot-2022-06-11-144935-react-firebase-assignment-09-firebaseapp-com.png",
      "https://i.ibb.co/YB8pQ8K/Opera-Snapshot-2022-06-11-145020-react-firebase-assignment-09-firebaseapp-com.png",
    ],
    link: {
      live: "https://react-firebase-assignment-09.firebaseapp.com",
      client: "https://github.com/mabuhanifa/snapmaster-client",
      server: "https://github.com/mabuhanifa/snapmaster-client",
    },
  },
];

export default projects;
